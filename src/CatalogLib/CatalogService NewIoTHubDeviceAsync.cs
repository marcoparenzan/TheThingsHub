...
{
    public async Task<(bool ok, X509Certificate2 result)> NewIoTHubDeviceAsync(ThingModel model, Func<X509Certificate2, Task<bool>> handleAsync)
    {
        ...
        var (_, deviceType) = await EnsurePropertyAsync(device, "deviceType", $"iothub-sas");
